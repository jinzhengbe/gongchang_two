...
LOG_FILE="test_extension_$(date +%s).log"
...
# 创建测试用户
TEST_USERNAME="ext_test_user_$(date +%s)"
TEST_PASSWORD="ext123456"
...
# 创建不同扩展名的文件
echo "PNG content" > test_image.PNG
echo "JPG content" > test_image.JPG
echo "JPEG content" > test_image.JPEG
echo "PDF content" > test_document.PDF
echo "STL content" > test_model.STL
...
log "6. 测试上传JPG文件（大写扩展名）..."
JPG_RESPONSE=$(curl -s -X POST "$BASE_URL/api/orders/$ORDER_ID/add-file" \
  -H "Authorization: Bearer $TOKEN" \
  -F "file=@test_image.JPG" \
  -F "type=image" \
  -F "description=测试JPG文件")

log "JPG上传响应: $JPG_RESPONSE"

# 提取JPG文件URL
JPG_URL=$(echo $JPG_RESPONSE | grep -o '"url":"[^"]*"' | cut -d'"' -f4)
if [ -n "$JPG_URL" ]; then
    log "JPG文件URL: $JPG_URL"
    # 检查是否保持了.JPG扩展名
    if [[ $JPG_URL == */uploads/*.JPG ]]; then
        log "✅ JPG文件保持了原始扩展名(.JPG)"
    else
        log "❌ JPG文件扩展名被修改了"
    fi
fi

log "7. 测试上传PDF文件（大写扩展名）..."
...
log "10. 清理测试文件..."
rm -f test_image.PNG test_image.JPG test_image.JPEG test_document.PDF test_model.STL test_no_extension
